...
static constexpr int animcount = 2;

AnimationFile* SA1MilesAnims[animcount];
NJS_ACTION SA2MilesActions[animcount];
...
void BetterMiles_Init() {
	...
	MilesFlySpeedValue = flyTest;

	//Remove the altitude nerf.
	WriteJump((void*)0x752DB0, Tails_FlyStartASM);

	WriteCall((void*)0x439b13, LoadNewAnimation);	
	WriteCall((void*)0x43cada, LoadNewAnimation);
